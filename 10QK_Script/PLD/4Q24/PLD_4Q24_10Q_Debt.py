...
# Adjust
pdf_path = "/home/fincofella/dev/Application/10QK_PDFs/PLD/PLD_2024_10K.pdf"
# Adjust
tables = tabula.read_pdf(pdf_path, pages=134, multiple_tables=True, stream=True, pandas_options={"header": None})
...
# Adjust
debt_maturity_df = (debt_maturity_df.loc[3:9, [0, 5, 7]].reset_index(drop=True))
debt_maturity_df[0] = (debt_maturity_df[0].astype(str).str.extract(r'(\d{4}|Thereafter|Subtotal)')[0]).replace("Subtotal", "Total")

# Adjust
debt_maturity_df = debt_maturity_df.rename(columns={0: "Year", 5: "Secured Debt", 7: "Total Debt"})
debt_maturity_df = debt_maturity_df.dropna(subset=["Year"])

# Adjust
for col in ["Secured Debt", "Total Debt"]:
    debt_maturity_df[col] = (pd.to_numeric(debt_maturity_df[col].astype(str).str.replace(r'[\$,()\s]', '', regex=True), errors="coerce").div(1_000).astype("int"))
