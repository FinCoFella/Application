...
CIK = '0001053507' # AMT CIK Number
HEADERS = {"User-Agent": "Nik (nd24@ic.ac.uk)"}
CSV = "AMT_10QK_EBITDA.csv"

RECENT_QUARTERS = 10
RECENT_YEARS = 5

def get_fact_df(data: dict, *fact_keys: str) -> pd.DataFrame:
    ...
    for key in fact_keys:
        try:
            ...
            continue
    if not all_items:
        print(f"No key(s) {fact_keys} found.")
        return pd.DataFrame()
    ...
    df = df.sort_values("filed", ascending=False).drop_duplicates(
        subset=["fy", "fp", "start", "end"], keep="first"
    )
...
def main():
    ...
    ie_raw = get_fact_df(json_data, "InterestExpenseNonoperating", "InterestExpense")
    te_raw = get_fact_df(json_data, "IncomeTaxExpenseBenefit")
    da_raw = get_fact_df(json_data, "DepreciationAmortizationAndAccretionNet")
